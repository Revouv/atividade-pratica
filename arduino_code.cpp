...
¦ Adaptação por: Angelo Luis Ferreira                     ¦
¦ Adaptação por: Guilherme Quaglio e Silva                ¦
+---------------------------------------------------------+
*/

// Definindo as notas para os sons (em Hz):
#define NOTE_D4  294
#define NOTE_G4  392
#define NOTE_A4  440
#define NOTE_A5  640
...
// Sequência de até 100 itens, começa vazia:
int sequencia[100] = {};

// Indica a rodada atual que o jogo se encontra:
int rodada_atual = 0;

// Indica o passo atual dentro da sequência, é usado enquanto a 
// sequência está sendo reproduzida:
int passo_atual_na_sequencia = 0;
 
/* 
+-------------------------------------------------------------+
¦ Indica o pino de áudio, LEDS e botões:                      ¦
¦ Os pinos de LEDS e botões estão em ordem, relacionados uns  ¦
¦ aos outros. Ou seja, o primeiro LED está relacionado ao     ¦
¦ primeiro botão. A ordem destas sequências também estão      ¦
¦ relacionadas a ordem dos tons.                              ¦
+-------------------------------------------------------------+
*/
...
void setup() {
  ...
  // Inicializando o random através de uma leitura da porta 
  // analógica.
...
void loop() {
  ...
  // Toca um som de início para anúnicar que o jogo está 
  // começando quando é a primeira rodada:
  if (rodada_atual == 0) {
    tocarSomDeInicio();
    delay(500);
  }
  ...
  while (!jogada_efetuada) {
    for (int i = 0; i <= 3; i++) {
      if (!digitalRead(pinosBotoes[i])) {
        // Diz qual foi o botao pressionado:
        botao_pressionado = i;
